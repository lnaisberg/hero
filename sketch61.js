...
let fontSize = p5Container.clientWidth * 0.08;
let maxLinetWidth = p5Container.clientWidth * 0.8;
let windowRatio = 2.2;

if(w/h > windowRatio){
    fontSize = p5Container.clientWidth * 0.025;
}

let letters = [];
let hiddenLetters = [];

let myMouseX, myMouseY;
...
//   canvas.style('z-index', -1);
  ...
  let options = {
    ...
    mConstraint.mouse.element.addEventListener('touchstart', mConstraint.mouse.mousedown, { passive: true });
    mConstraint.mouse.element.addEventListener('touchmove', (e) => {
    if (mConstraint.body) {
        mConstraint.mouse.mousemove(e);
    }
    });
    mConstraint.mouse.element.addEventListener('touchend', (e) => {
    if (mConstraint.body) {
        mConstraint.mouse.mouseup(e);
    }
    });
...
function calcFontSize(){
    if(width > 1000){
      fontSize = width * 0.05;
      maxLinetWidth = width * 0.6;
      if(w/h > windowRatio){
        fontSize = p5Container.clientWidth * 0.03;
      }
    }
    ...
    fontSize = width * 0.06;
      maxLinetWidth = width * 0.6;
      if(w/h > windowRatio){
        fontSize = p5Container.clientWidth * 0.04;
      }
    }
  
    if(width < 700 && width > 450){
      fontSize = width * 0.08;
      maxLinetWidth = width * 0.7;
      if(w/h > windowRatio){
        fontSize = p5Container.clientWidth * 0.04;
      }
    }
...
function placeText(){
    ...
    for (let i = 0; i < message.length; i++) {
      ...
      if(char == " "){
        ...
        if(wordLength + currentLine > maxLinetWidth){
          posX = width * 0.15;
          posY += fontSize * 1.2;
          lineLength.push(currentLine);
          currentLine = 0;
          lineCount++;
        }
      }
    ...
    let totalHeight = fontSize * 1.2 * (lineCount-1);
    ...
    for (let i = 0; i < message.length; i++) {
      ...
      if(char == " "){
        ...
        if(wordLength + currentLine > maxLinetWidth){
          lineCounter++;
          posX = (width - lineLength[lineCounter])/2;
          // posX = width * 0.15;
          posY += fontSize * 1.2;
          currentLine = 0;
        }
...
function draw() {
    let transformW = p5Container.getBoundingClientRect().width;
    let transformH = p5Container.getBoundingClientRect().height;
    let left = (w - transformW)/2;
    let top = (h - transformH)/2;
    myMouseX = map(mouseX, 0, w, -left, w+left);
    myMouseX += (w - transformW)/2;
    myMouseY = map(mouseY, 0, h, -top, h+top);
    myMouseY += (h - transformH)/2;
    mConstraint.mouse.position.x = myMouseX;
    mConstraint.mouse.position.y = myMouseY;
    // ellipse(myMouseX, myMouseY, 3, 3);
...
//   h = windowHeight;
  resizeCanvas(w, h);
  fontSize = p5Container.clientWidth * 0.08;
  maxLinetWidth = p5Container.clientWidth * 0.8;
...
class Letter{
  ...
  checkHover(){
      if(myMouseX > this.x && myMouseX < this.x + this.w && myMouseY > this.y - this.fontSize/2 && myMouseY < this.y + this.fontSize/2){
